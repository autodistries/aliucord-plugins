...
class UserLookup : Plugin() {
    ...
    override fun start(_ctx: Context) {
        ...
        commands.registerCommand("whois", "Look up a user", mentionOrId) { ctx ->
            ...
            val guildMember : GuildMember? = profile.c()?.let { GuildMember.Companion.`from$default`(GuildMember.Companion, it, ctx.currentChannel.guildId, emptyMap<Long, GuildRole>(), StoreStream.getGuilds(), 0, null) }
